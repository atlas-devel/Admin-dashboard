    ...
    <div className=" overflow-auto flex-1 ">
      {items.map(({ id, name, link, Icon }: SidebarInfo) => (
        <div key={id} className="flex items-center group cursor-pointer ">
          ...
          <Link
            to={link}
            className="flex  items-center gap-4 transition-opacity duration-400  mb-1 text-primary-text group-hover:bg-secondary-color group-hover:text-white py-4 w-full mx-5 rounded-md px-3"
          >
